...
export const useAuthStore = defineStore('auth', () => {
    ...
    const fallbackUserId = ref<string | null>(null)
    const fallbackUserName = ref<string | null>(null)

    let keycloakInstance: Keycloak | null = null

    const userId = computed(() => {
        if (keycloakInstance && user.value?.sub) {
    ...
    function setFallbackUser(userId: string, userName: string) {
        fallbackUserId.value = userId
        fallbackUserName.value = userName
    ...
    function logout(redirectUri?: string) {
        ...
        fallbackUserId.value = null
        fallbackUserName.value = null
        user.value = null
        token.value = null
