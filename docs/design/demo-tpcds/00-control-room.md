...
- 2026-07-09 · **W-1** (S10, Bora) · Wrap-up deliverables = **`demo-transcript.md` (the design artifact — the transcript IS the design) + `olymp/clusters/hartland/plan-cluster.md` (the demo-cluster plan)**. No `design.md`/`detailed-design.md`; no separate architecture artefact (the architecture is Kantheon itself). `07-f` remains the presenter-operations companion (timing/fallbacks/rehearsal); beat dialogue is canonical in the transcript, synced at R0. · Rejected: the default design.md+detailed-design.md pair (skill convention; superseded per the 2026-06-27 planning-doc-structure rule).
- 2026-07-09 · **E-1a** (amends E-1 naming lean, Bora) · The olymp overlay is **`clusters/hartland`** (root-app-hartland, kube-context `hartland`), not `showcase`. · Why: Bora named it in the wrap-up brief; the demo world and its cluster share the name.
- 2026-07-09 · **S-8a** (amends S-8's parenthetical; resolves Q-6) · r13 on hartland is clean: five DCs at ~20% Marketplace share each (±1%). **Memphis DC := the NULL-named warehouse** (highest 2025 share, 20.13%, $146.1M — and it needed a display name anyway); the four gibberish-named rows get Columbus/Dallas/Reno/Allentown. · Q-4 guidance recorded in `demo-transcript.md` App. B: 60% deletion ⇒ H2 ≈ −8.4% (Nov −12% ✓); to hit the −10..12% H2 headline, raise deletion to ~75–80%, widen the window, or re-label the KPI — decide at seed build.
...
- **Q-4**: seed magnitudes tuning — feeds `07-f` R0 (script number-freeze). **Guidance locked (S-8a / `demo-transcript.md` App. B):** 60% Memphis deletion ⇒ H2 ≈ −8.4%, Nov ≈ −12%; pick deletion ~75–80% / wider window / re-labeled KPI at seed build.
...
- ~~**Q-6**: `cs_warehouse_sk` slice cleanliness~~ — RESOLVED 2026-07-09 by r13 on hartland (S-8a): clean, five DCs at ~20% ± 1%.
...
| 2026-07-09 | S10 | Wrap-up (surgery landed) | Bora: redate + hartland recon baseline committed, pre-seed dump stashed. **W-1 (transcript-as-design), E-1a (`clusters/hartland`), S-8a (Memphis = ex-NULL DC; Q-6 ✅; Q-4 arithmetic guidance)**. Deliverables written: **`demo-transcript.md`** (design artifact, real baseline numbers, Requires-rollup) + **`olymp/clusters/hartland/plan-cluster.md`** (phases H1–H5, gates G1–G5). **Design effort COMPLETE — next: /planning task lists (kantheon: hartland repo + seeds + run-set; olymp: H1 start)** |
