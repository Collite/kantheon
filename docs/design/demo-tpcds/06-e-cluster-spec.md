...
5. `pg_dump -Fc hartland` → **demo dump** — the versioned artifact the showcase cluster restores from; store next to the .dat files in `tpcds-staging` (or a `hartland-dumps` bucket).
...
- **`demo-reset`** (just recipe, showcase-only): truncate *session* state — iris sessions/SSE, Pythia investigations, feedback — while **preserving** standing fixtures: the pinned dashboard, the Hebe Monday routine, Keycloak users, and the `hartland` warehouse (read-only, never dirtied). Beat 1 depends on the routine surviving reset; the *briefing inbox item* is re-generated per rehearsal (fire the routine manually = the pre-show step).
