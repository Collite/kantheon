...
> **The "read spine" was extracted to `tatrman-server` (2026-07, SV-P0/P1).** The eight forked query-path services in the table below — Ariadne, Theseus, Echo, Kadmos, Proteus, Kyklop, Argos, Prometheus — were **moved out of kantheon** into the open-source [`tatrman-server`](https://github.com/Collite/tatrman-server) repo and renamed to functional names (`services/ariadne` and its dirs no longer exist here). Mapping — including the wire packages and MCP edges: Ariadne→**Veles** (`ariadne.v1`→`meta.v1`, `ariadne-mcp`→`meta-mcp`); Theseus→**query** (`theseus.v1`→`query.v1`, `theseus-mcp`→`query-mcp`); Echo→**lex-matcher** (`echo.v1`→`fuzzy.v1`, `echo-mcp`→`lex-matcher-mcp`); Kadmos→**nlp** (`kadmos.v1`→`nlp.v1`, `kadmos-mcp`→`nlp-mcp`); Proteus→**translate** (`proteus.v1`→`translate.v1`); Kyklop→**dispatch** (`kyklop.v1`→`dispatch.v1`); Argos→**validate** (`argos.v1`→`validate.v1`); Prometheus→**llm-gateway** (`prometheus.v1`→`llm.v1`). **Charon, Metis, report-renderer, and `capabilities-mcp` stay in kantheon.** The rows below are kept with their fork-era detail as history; the persona names and `fork:`-origin annotations are naming history, not a current-location claim.

| Module | Role | Stack |
|---|---|---|
| `services/ariadne` (+ `tools/ariadne-mcp`) — **→ `tatrman-server`: Veles (+ `meta-mcp`)** | Model graph / metadata; serves Shem **model + prompts** (`GetModel` / `GetPrompts`) from the `ai-models` repo (fork: `infra/metadata`) | Kotlin + Ktor + gRPC |
| `services/theseus` (+ `tools/theseus-mcp`) — **→ `tatrman-server`: query (+ `query-mcp`)** | Query orchestrator + plan cache; the data-path edge (`theseus-mcp`) agents call under OBO (fork: `services/query-runner`) | Kotlin + Ktor + gRPC |
| `services/echo` (+ `tools/echo-mcp`) — **→ `tatrman-server`: lex-matcher (+ `lex-matcher-mcp`)** | Czech-aware fuzzy matcher (fork: `services/fuzzy-matcher`) | Kotlin + Ktor + gRPC |
| `services/kadmos` (+ `tools/kadmos-mcp`) — **→ `tatrman-server`: nlp (+ `nlp-mcp`)** | NLP foundation — Stanza / spaCy / NameTag / MorphoDiTa (fork: `infra/nlp`) | **Python** + gRPC |
| `services/proteus` — **→ `tatrman-server`: translate** | Translator: lang ↔ RelNode ↔ SQL; consumes the `Collite/modeler` TTR toolchain (fork: `services/translator`) | Kotlin + gRPC |
| `services/kyklop` — **→ `tatrman-server`: dispatch** | Worker dispatcher — routes plans to the Kyklops workers (fork: `services/dispatcher`) | Kotlin + Ktor + gRPC |
| `services/argos` — **→ `tatrman-server`: validate** | Validator + RLS policy; **sql-security folded in**; reads roles from the forwarded bearer (`argos.roleSource = bearer \| whois`, default bearer) (fork: `services/validator` + `infra/sql-security`) | Kotlin + Ktor |
| `services/prometheus` — **→ `tatrman-server`: llm-gateway** | LLM gateway (fork: `infra/llm-gateway`, forked as-is). *(Distinct from the Prometheus monitoring stack / Micrometer / `/metrics`, which stays.)* | **Spring Boot** |
...
> **All three workers extracted to `tatrman-server` (2026-07)** as `worker-{mssql,polars,postgres}` (Brontes→worker-mssql, Steropes→worker-polars, Arges→worker-postgres). Kept here as naming history.

| Module | Role | Stack |
|---|---|---|
| `workers/brontes` — **→ `tatrman-server`: worker-mssql** | MSSQL worker — DB → Arrow extract / Arrow → DB ingest (fork: `workers/mssql`) | Kotlin + gRPC |
| `workers/steropes` — **→ `tatrman-server`: worker-polars** | Polars DataFrame worker — scan plans, materialisation, read-out (fork: `workers/polars`) | **Python** + Polars + gRPC |
| `workers/arges` — **→ `tatrman-server`: worker-postgres** | Postgres worker — re-homes ai-platform `workers/postgres` (needed by Midas P3.2) | Kotlin + gRPC |

#### 2.d Infrastructure — the technical wave (no personas, fork Phase 5)

> **`whois`, `health`, `backstage` extracted to `tatrman-server` (2026-07)** — whois→**identity** (the `roleSource: bearer|whois` config vocabulary and the `whois-common` lib name are retained). **`landing` stays in kantheon.**

| Module | Role | Stack |
|---|---|---|
| `infra/whois` — **→ `tatrman-server`: identity** | User/role directory + OPA bundle server; own Postgres; optional Argos role-enrichment source (fork: `infra/whois`) | Kotlin + Ktor + Postgres |
...
> **Stale as of the 2026-07 extraction.** The tree below is the fork-era target layout. Since then the **read spine** (`services/{ariadne,theseus,echo,kadmos,proteus,kyklop,argos,prometheus}` + their `tools/*-mcp` wrappers), the **workers** (`workers/{brontes,steropes,arges}`), and the **technical-wave infra** `infra/{whois,health,backstage}` were extracted to the open-source [`tatrman-server`](https://github.com/Collite/tatrman-server) repo and renamed to functional names (Veles / {query,lex-matcher,nlp,translate,dispatch,validate,llm-gateway} + worker-{mssql,polars,postgres}; whois→identity). **Those directories no longer exist in kantheon.** What remains under `services/` is `charon`, `metis`, `report-renderer`; `frontends/landing` stays; the agents, `tools/capabilities-mcp`, `shared/`, and the domain modules are unaffected. The annotations below mark the moved subtrees.
...
│   ├── ariadne/                         # → tatrman-server: Veles             + tools/meta-mcp
│   ├── theseus/                         # → tatrman-server: query         + tools/query-mcp
│   ├── echo/                            # → tatrman-server: lex-matcher         + tools/lex-matcher-mcp
│   ├── kadmos/                          # → tatrman-server: nlp (Python)  + tools/nlp-mcp
│   ├── proteus/                         # → tatrman-server: translate
│   ├── kyklop/                          # → tatrman-server: dispatch
│   ├── argos/                           # → tatrman-server: validate
│   ├── prometheus/                      # → tatrman-server: llm-gateway (Spring Boot)
...
│   ├── brontes/                         # → tatrman-server: worker-mssql
│   ├── steropes/                        # → tatrman-server: worker-polars (Python)
│   └── arges/                           # → tatrman-server: worker-postgres
├── infra/                               # technical wave — whois/health/backstage EXTRACTED → tatrman-server (2026-07)
│   ├── whois/                           # → tatrman-server: identity (user/role directory + OPA bundles)
...
│   └── {charon,metis}-mcp/              # thin MCP wrappers (ariadne/theseus/echo/kadmos-mcp extracted → tatrman-server as {meta,query,lex-matcher,nlp}-mcp)
├── shared/
│   ├── proto/
│   │   └── src/main/proto/
...
> **Read spine since extracted (2026-07, SV-P0/P1).** Everything this section describes as "in-repo" or "forked in" — Themis's kadmos-mcp/echo-mcp/Prometheus edges, and the theseus-mcp → Theseus → Proteus/Argos/Kyklop → Brontes/Steropes data path — was later **moved out of kantheon** into the open-source [`tatrman-server`](https://github.com/Collite/tatrman-server) repo and renamed to functional names (Ariadne→Veles, Theseus→query, Echo→lex-matcher, Kadmos→nlp, Proteus→translate, Kyklop→dispatch, Argos→validate, Prometheus→llm-gateway, workers→worker-*; whois→identity, health/backstage too). Read the "in-repo" claims below as fork-era history; those services now run from tatrman-server.
