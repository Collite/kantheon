...
- [~] **T4 [K/O] — `golem-erp` (fixture agent-showcase).** ✅ **Authored + local-green 2026-07-08; awaiting one gated bp-dsk run.** Reframed to the **agent showcase** (Bora, 2026-07-08): the fixture query leg returns `detection_failed` not rows (`theseus-runquery` finding), so golem-erp proves the **Golem agent turn**, not real query data (that is `tpcds-query`'s job). **No image rebuilds** — the golem-erp Shem points at the existing bundled Ariadne `accounting` area, and is delivered as a per-run **ConfigMap** (new olymp harness `shems:` field → `<agent_id>-shem`, mounted via golem `shem.configMapName`; prompts fall back to the golem image classpath `prompts/cs/*`).
  ...
  - **Gated (Bora):** `just it-bp-dsk golem-erp` — expect the two admission assertions GREEN. Then flip `answerTurnLive=true` + re-run to exercise the golem→prometheus→WireMock LLM roundtrip (Spring AI 2.0.0-M2 Anthropic wire shape / gateway path `/v1/chat/completions` vs `/api/v1/chat/completions` may need one live tweak). **No image builds needed** — reuse the standing `:testing` images.
