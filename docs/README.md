...
- **kantheon** (`/Users/bora/Dev/kantheon`) — this repo: the agent constellation (Iris, Themis, Pythia, Golem instances, Hebe, `capabilities-mcp`) **and the new self-contained platform** (Charon, Metis, and the fork roster: Ariadne, Theseus, Echo, Kadmos, Proteus, Kyklop, Argos, Prometheus, Brontes, Steropes). See [`architecture/fork/architecture.md`](./architecture/fork/architecture.md).
