package shared.translator.schema
...
import shared.translator.framework.FixtureModel
import shared.translator.framework.InMemoryModelHandle
    ...
    StringSpec({
        ...
        val custTableQn =
            QualifiedName
                ...
                .build()
        val custTable =
            shared.translator.framework.ModelTable(
                qname = custTableQn,
                columns =
                    listOf(
                        shared.translator.framework.ModelColumn("CUST_ID", SurfaceType.INT, nullable = false),
                        shared.translator.framework.ModelColumn("CUST_NAME", SurfaceType.TEXT),
                        shared.translator.framework.ModelColumn("ACTIVE", SurfaceType.INT),
                    ),
            )
        ...
        fun queryBackedModel(
            bodies: Map<QualifiedName, shared.translator.framework.SavedQueryBody> =
                mapOf(
                    filterQueryStoredQn to
                        shared.translator.framework.SavedQueryBody(filterBody(), emptyList(), emptyList()),
                ),
            ...
            InMemoryModelHandle(
                ...
                savedQueries = listOf(shared.translator.framework.ModelSavedQuery(filterQueryStoredQn)),
        ...
        "entity → query whose body re-scans the same entity → entity_query_mapping_cycle" {
            ...
            val cyclicBodies =
                mapOf(
                    filterQueryStoredQn to
                        shared.translator.framework.SavedQueryBody(erScan("customer"), emptyList(), emptyList()),
                )
        ...
        "attribute renames → TableScan output_columns: name=DB col, alias=ER attr, type from DB column" {
            ...
            val tableForRenames =
                shared.translator.framework.ModelTable(
                    ...
                    columns =
                        listOf(
                            shared.translator.framework.ModelColumn("ID_COL", SurfaceType.INT, nullable = false),
                            shared.translator.framework.ModelColumn("NAME_COL", SurfaceType.TEXT),
                        ),
                )
