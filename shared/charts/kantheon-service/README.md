# `kantheon-service` — shared Helm library chart
...
```
<module>/k8s/
├── Chart.yaml            # type: application + a dependency on kantheon-service
├── values.yaml           # the module's defaults (image, ports, resources, env inputs)
└── templates/
    ├── deployment.yaml   # {{ include "kantheon-service.deployment" . }}
    ├── service.yaml      # {{ include "kantheon-service.service" . }}
...
dependencies:
  - name: kantheon-service
    version: 0.1.0
    repository: "file://../../../shared/charts/kantheon-service"
```
...
| Template | Emits |
|---|---|
| `kantheon-service.deployment` | backend Deployment (Kotlin/Ktor · agent · worker · Python) |
| `kantheon-service.service` | backend ClusterIP Service (`http`; `grpc` gated on `.Values.ports.grpc`) |
| `kantheon-service.helpers` (`.name`/`.fullname`/`.labels`/`.selectorLabels`/`.image`) | naming + labels, keyed off `.Chart.*` |
| `kantheon-service.fe-deployment` / `.fe-service` / `.fe-configmap` / `.httproute` | FE-nginx shape (see `frontends/iris/k8s`) |
...
`kantheon-service.volumeMounts` and `kantheon-service.volumes` default to empty.
