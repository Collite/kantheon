...
class CatchupSpec : StringSpec({
    ...
    "run_once_on_wake fires exactly once regardless of how many ticks were missed" {
        ...
        val plan = CatchupPlanner.plan(listOf(routine(CatchupPolicy.RUN_ONCE_ON_WAKE, 100)), now = 1000, coalesce = false)
    ...
    "run_all_missed fires once per owed tick (incl. the one due exactly now)" {
        ...
        val plan = CatchupPlanner.plan(listOf(routine(CatchupPolicy.RUN_ALL_MISSED, 100)), now = 280, coalesce = false)
    ...
    "run_all_missed with coalesce collapses the backlog to one fire" {
        val plan = CatchupPlanner.plan(listOf(routine(CatchupPolicy.RUN_ALL_MISSED, 100)), now = 10_000, coalesce = true)
        plan.fires shouldHaveSize 1
    }

    "skip produces no owed fire but advances next_run_at" {
        val plan = CatchupPlanner.plan(listOf(routine(CatchupPolicy.SKIP, 100)), now = 280, coalesce = false)
        plan.fires shouldHaveSize 0
        (plan.advancedNextRunAt["r"]!! > 280) shouldBe true
    }

    "a not-yet-due routine is untouched" {
        val plan = CatchupPlanner.plan(listOf(routine(CatchupPolicy.RUN_ALL_MISSED, 5000)), now = 1000, coalesce = false)
        plan.fires shouldHaveSize 0
        plan.advancedNextRunAt["r"] shouldBe 5000
    }

    "a routine due exactly now owes one tick" {
        val plan = CatchupPlanner.plan(listOf(routine(CatchupPolicy.RUN_ALL_MISSED, 1000)), now = 1000, coalesce = false)
        plan.fires shouldHaveSize 1
    }
})
