...
class OutboxSpec : StringSpec({
    ...
    "drain sends pending rows and removes them; a second drain is a no-op" {
        runTest {
            ...
            outbox.drain { sent.add(it.key); true }.sent shouldBe 2
            outbox.pendingCount() shouldBe 0
            outbox.drain { sent.add(it.key); true }.sent shouldBe 0 // nothing left
    ...
    "a send failure leaves the row for retry and preserves per-destination order" {
        runTest {
            ...
            outbox.drain { order.add(it.key); true }.sent shouldBe 2
