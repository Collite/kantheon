 ...
 */
class FallbackRouterSpec :
    StringSpec({

        "online: everything uses the gateway" {
            FallbackRouter.decide(
                LlmSource.GATEWAY_WITH_BYOK_FALLBACK,
                breakerOpen = false,
                TurnKind.OWN_ROUTINE,
            ) shouldBe RoutingDecision.UseGateway
            FallbackRouter.decide(
                LlmSource.GATEWAY_WITH_BYOK_FALLBACK,
                breakerOpen = false,
                TurnKind.CONSTELLATION,
            ) shouldBe RoutingDecision.UseGateway
        }
        ...
        "offline + own routine → byok fallback (Hebe stays useful)" {
            FallbackRouter.decide(
                ...
                breakerOpen = true,
                TurnKind.OWN_ROUTINE,
            ) shouldBe RoutingDecision.UseByokFallback
        }
        ...
        "offline + constellation turn → defer (never falls back, never silent)" {
            FallbackRouter.decide(
                ...
                breakerOpen = true,
                TurnKind.CONSTELLATION,
            ) shouldBe RoutingDecision.Defer
        }
        ...
        "byok source is always local" {
            FallbackRouter.decide(
                ...
                breakerOpen = false,
                TurnKind.OWN_ROUTINE,
            ) shouldBe RoutingDecision.UseByok
            FallbackRouter.decide(
                LlmSource.BYOK,
                breakerOpen = true,
                TurnKind.OWN_ROUTINE,
