...
class ChatDelegate(
    ...
    private val observer: org.tatrman.kantheon.hebe.api.Observer,
    private val systemPrompt: String,
    private val toolsProvider: suspend (userMessage: String) -> List<org.tatrman.kantheon.hebe.api.ToolSpec>,
    ...
    override suspend fun callLlm(
        ...
        // Bind the turn id into the coroutine context so GatewayClient can stamp
        // `X-Turn-Ref` on the gateway call. No-op for BYOK/local providers, which
        // never read the context key.
        val events = withTurnRef(ctx.turnId) { llmProvider.chat(request).toList() }
        ...
        // cost of every final turn — the headline cost-attribution deliverable.
        // Recording at the end of each callLlm fires exactly once per LLM call.
        recordCallCost()
    ...
    private fun toToolContext(ctx: ReasoningContext): ToolContext =
        object : ToolContext {
            ...
            override val workspace: org.tatrman.kantheon.hebe.api.workspace.WorkspacePath = ctx.workspace
            override val approvalGate: org.tatrman.kantheon.hebe.api.ApprovalGate = ctx.approvalGate
            override val observer: org.tatrman.kantheon.hebe.api.Observer = ctx.observer
            override val secretLookup: org.tatrman.kantheon.hebe.api.SecretLookup = ctx.secretLookup
        }
