...
class ChannelAndConsoleSpec :
    StringSpec({
        ...
        "OIDC verifier rejects issuer mismatch, expiry, and bad signature" {
            val v =
                OidcSessionVerifier(
                    "https://kc/realms/kantheon",
                    "hebe",
                    validateSignature = { true },
                    now = { 1000 },
                )
            v
                .verify("t", mapOf("iss" to "https://evil", "exp" to 2000, "sub" to "x"))
                .shouldBeInstanceOf<ConsoleAuthResult.Rejected>()
            v
                .verify("t", mapOf("iss" to "https://kc/realms/kantheon", "aud" to "hebe", "exp" to 500, "sub" to "x"))
                .shouldBeInstanceOf<ConsoleAuthResult.Rejected>()
            OidcSessionVerifier(
                "https://kc/realms/kantheon",
                "hebe",
