 ...
 * **real** themis-mcp pod wired to real Nlp (NLP) + Fuzzy (fuzzy) + capabilities-mcp (agent
 * manifests), with the LLM upstream (via LLM gateway) available at WireMock. Gated by
 ...
 *    sets `routingEnabled=false`, so it runs NLP (Nlp) + the resolution graph and returns a
 ...
 *    capabilities-mcp, reaches Nlp, runs the graph). This is the deliverable that lands green
 ...
 *    stubs shaped to each leg's decode contract. Author those from the first bp-dsk run's LLM gateway
 ...
 *  - Services (real): themis-mcp, capabilities-mcp (seed agent manifests → themis boots), nlp
 *    (NLP), fuzzy (fuzzy), llm-gateway (Spring `test` profile → H2, LLM base-url'd at WireMock).
 *  - Platform: `wiremock` (LLM upstream stub — empty on the robust tier; the graph degrades).
 *  - No mssql / query chain — routing classifies the question; it does not execute a query.
...
class ThemisRoutingIntegrationSpec :
    StringSpec({
        ...
        // proves no node threw AND that Nlp NLP ran — the graph builds its context from the NLP
        ...
        // Nlp, and executes the resolution graph (degrading gracefully on the stub-less LLM legs).
        //
        ...
        // degraded outcome with an empty WireMock. `isError=false` already covers "Nlp ran".
        "resolve returns a well-formed terminal outcome"
            .config(enabled = contextLive) {
                val handle = contextHandle()

                // Retry across a cold Nlp: the first attempt triggers its lazy model-load (paired
                // with the raised NLP timeout in themis-mcp.values.yaml + more Nlp memory); if that
